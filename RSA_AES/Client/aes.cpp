...
int AES::Encrypt(char* _in, int _length, char* _out, char* iv) {
...
int AES::Decrypt(char* _in, int _length, char* _out, char* iv) {
	...
	for (int j = 0; j < rounds; j++) {
		...
		this->InvCipher(ciphertext, output);
...
char* AES::Padding(char* _in)
{
	...
	if ((strlen(_in)) % 16 != 0)
	{
		...
		int len = ((strlen(_in) / 16) + 1) * 16;
		padded = new char[len];
		memset(padded, 46, len);

		memcpy(padded, _in, strlen(_in));
		padded[len] = '\0';
		return padded;
	}
	else
...
//{
//	char mingwen[] = "hellohellohellohhellohellohellohhellohellohelloh123";
//	char miwen_hex[1024];
//	char* temp = Padding(mingwen);
//
//	char result[1024];
...
//	len = aes.Encrypt(temp, strlen(temp), miwen_hex, iv);
//	cout << miwen_hex << endl;
//
//	aes.Decrypt(miwen_hex, len, result, iv);
//	printf("%s\n", result);
//
//	return 0;
//}
