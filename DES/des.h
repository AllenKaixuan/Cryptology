#include <bitset>
#include <iostream>
#include <unordered_map>
#include <vector>
#include <algorithm>
using namespace std;
#define NUM 8
// 初始置换表
int IP[64] = {58, 50, 42, 34, 26, 18, 10, 2, 60, 52, 44,
			  36, 28, 20, 12, 4, 62, 54, 46, 38, 30, 22,
			  14, 6, 64, 56, 48, 40, 32, 24, 16, 8, 57,
			  49, 41, 33, 25, 17, 9, 1, 59, 51, 43, 35,
			  27, 19, 11, 3, 61, 53, 45, 37, 29, 21, 13,
			  5, 63, 55, 47, 39, 31, 23, 15, 7};

// 扩展置换运算
int expand[48] = {32, 1, 2, 3, 4, 5, 4, 5, 6, 7, 8, 9,
				  8, 9, 10, 11, 12, 13, 12, 13, 14, 15, 16, 17,
				  16, 17, 18, 19, 20, 21, 20, 21, 22, 23, 24, 25,
				  24, 25, 26, 27, 28, 29, 28, 29, 30, 31, 32, 1};

// S盒
int S[8][4][16] = {
	{14, 4, 13, 1, 2, 15, 11, 8, 3, 10, 6, 12, 5,
	 9, 0, 7, 0, 15, 7, 4, 14, 2, 13, 1, 10, 6,
	 12, 11, 9, 5, 3, 8, 4, 1, 14, 8, 13, 6, 2,
	 11, 15, 12, 9, 7, 3, 10, 5, 0, 15, 12, 8, 2,
	 4, 9, 1, 7, 5, 11, 3, 14, 10, 0, 6, 13},
	{15, 1, 8, 14, 6, 11, 3, 4, 9, 7, 2, 13, 12,
	 0, 5, 10, 3, 13, 4, 7, 15, 2, 8, 14, 12, 0,
	 1, 10, 6, 9, 11, 5, 0, 14, 7, 11, 10, 4, 13,
	 1, 5, 8, 12, 6, 9, 3, 2, 15, 13, 8, 10, 1,
	 3, 15, 4, 2, 11, 6, 7, 12, 0, 5, 14, 9},

	{10, 0, 9, 14, 6, 3, 15, 5, 1, 13, 12,
	 7, 11, 4, 2, 8, 13, 7, 0, 9, 3, 4,
	 6, 10, 2, 8, 5, 14, 12, 11, 15, 1, 13,
	 6, 4, 9, 8, 15, 3, 0, 11, 1, 2, 12,
	 5, 10, 14, 7, 1, 10, 13, 0, 6, 9, 8,
	 7, 4, 15, 14, 3, 11, 5, 2, 12},
	{7, 13, 14, 3, 0, 6, 9, 10, 1, 2, 8, 5, 11,
	 12, 4, 15, 13, 8, 11, 5, 6, 15, 0, 3, 4, 7,
	 2, 12, 1, 10, 14, 9, 10, 6, 9, 0, 12, 11, 7,
	 13, 15, 1, 3, 14, 5, 2, 8, 4, 3, 15, 0, 6,
	 10, 1, 13, 8, 9, 4, 5, 11, 12, 7, 2, 14},
	{2, 12, 4, 1, 7, 10, 11, 6, 8, 5, 3, 15, 13,
	 0, 14, 9, 14, 11, 2, 12, 4, 7, 13, 1, 5, 0,
	 15, 10, 3, 9, 8, 6, 4, 2, 1, 11, 10, 13, 7,
	 8, 15, 9, 12, 5, 6, 3, 0, 14, 11, 8, 12, 7,
	 1, 14, 2, 13, 6, 15, 0, 9, 10, 4, 5, 3},
	{12, 1, 10, 15, 9, 2, 6, 8, 0, 13, 3, 4, 14,
	 7, 5, 11, 10, 15, 4, 2, 7, 12, 9, 5, 6, 1,
	 13, 14, 0, 11, 3, 8, 9, 14, 15, 5, 2, 8, 12,
	 3, 7, 0, 4, 10, 1, 13, 11, 6, 4, 3, 2, 12,
	 9, 5, 15, 10, 11, 14, 1, 7, 6, 0, 8, 13},
	{4, 11, 2, 14, 15, 0, 8, 13, 3, 12, 9, 7, 5,
	 10, 6, 1, 13, 0, 11, 7, 4, 9, 1, 10, 14, 3,
	 5, 12, 2, 15, 8, 6, 1, 4, 11, 13, 12, 3, 7,
	 14, 10, 15, 6, 8, 0, 5, 9, 2, 6, 11, 13, 8,
	 1, 4, 10, 7, 9, 5, 0, 15, 14, 2, 3, 12},
	{13, 2, 8, 4, 6, 15, 11, 1, 10, 9, 3, 14, 5,
	 0, 12, 7, 1, 15, 13, 8, 10, 3, 7, 4, 12, 5,
	 6, 11, 0, 14, 9, 2, 7, 11, 4, 1, 9, 12, 14,
	 2, 0, 6, 10, 13, 15, 3, 5, 8, 2, 1, 14, 7,
	 4, 10, 8, 13, 15, 12, 9, 0, 3, 5, 6, 11}};

// 置换运算P
int P[32] = {16, 7, 20, 21, 29, 12, 28, 17, 1, 15, 23,
			 26, 5, 18, 31, 10, 2, 8, 24, 14, 32, 27,
			 3, 9, 19, 13, 30, 6, 22, 11, 4, 25};

// 逆初始置换IP
int IP_trans[64] = {40, 8, 48, 16, 56, 24, 64, 32, 39, 7, 47,
					15, 55, 23, 63, 31, 38, 6, 46, 14, 54, 22,
					62, 30, 37, 5, 45, 13, 53, 21, 61, 29, 36,
					4, 44, 12, 52, 20, 60, 28, 35, 3, 43, 11,
					51, 19, 59, 27, 34, 2, 42, 10, 50, 18, 58,
					26, 33, 1, 41, 9, 49, 17, 57, 25};
// 置换选择1
int PC_1[56] = {57, 49, 41, 33, 25, 17, 9, 1, 58, 50, 42, 34,
				26, 18, 10, 2, 59, 51, 43, 35, 27, 19, 11, 3,
				60, 52, 44, 36, 63, 55, 47, 39, 31, 23, 15, 7,
				62, 54, 46, 38, 30, 22, 14, 6, 61, 53, 45, 37,
				29, 21, 13, 5, 28, 20, 12, 4};

// 置换选择2
int PC_2[48] = {14, 17, 11, 24, 1, 5, 3, 28,
				15, 6, 21, 10, 23, 19, 12, 4,
				26, 8, 16, 7, 27, 20, 13, 2,
				41, 52, 31, 37, 47, 55, 30, 40,
				51, 45, 33, 48, 44, 49, 39, 56,
				34, 53, 46, 42, 50, 36, 29, 32};

vector<string> round_key;

string hex_bin(const unsigned char *s)
{
	string bin = "";
	for (int i = 0; i < NUM; i++)
	{

		int temp = s[i];
		bitset<8> bitvec(temp);
		bin += bitvec.to_string();
	}
	return bin;
}



string bin_hex(string s)
{
	string hex = "";
	unordered_map<string, string> table;
	table["0000"] = "0";
	table["0001"] = "1";
	table["0010"] = "2";
	table["0011"] = "3";
	table["0100"] = "4";
	table["0101"] = "5";
	table["0110"] = "6";
	table["0111"] = "7";
	table["1000"] = "8";
	table["1001"] = "9";
	table["1010"] = "A";
	table["1011"] = "B";
	table["1100"] = "C";
	table["1101"] = "D";
	table["1110"] = "E";
	table["1111"] = "F";
	for (int i = 0; i < s.length(); i += 4)
	{
		string ch = "";

		ch += s[i];
		ch += s[i + 1];
		ch += s[i + 2];
		ch += s[i + 3];
		hex += table[ch];
	}
	return hex;
}

string dec_bin(int a)
{
	string s = "";
	s += char(a / 8 + '0');
	a = a % 8;
	s += char(a / 4 + '0');
	a = a % 4;
	s += char(a / 2 + '0');
	a = a % 2;
	s += char(a + '0');
	return s;
}

string replace(string a, int *arr, int n)
{
	string s = "";
	for (int i = 0; i < n; i++)
	{
		s += a[arr[i] - 1];
	}
	return s;
}

string shift_left(string a, int shift)
{
	string s1 = "";
	string s2 = "";
	s1.assign(a, shift, a.size() - 1);
	s2.assign(a, 0, shift);
	s1 += s2;
	return s1;
}

string xor_(string a, string b)
{
	string ans = "";
	for (int i = 0; i < a.size(); i++)
	{
		if (a[i] == b[i])
			ans += "0";
		else
			ans += "1";
	}
	return ans;
}

string plt_utility(string m, vector<string> round_key)
{

	// IP置换
	m = replace(m, IP, 64);

	// 分为两半
	string left = m.substr(0, 32);
	string right = m.substr(32, 32);

	for (int i = 0; i < 16; i++)
	{
		// 扩展置换
		string right_expanded = replace(right, expand, 48);

		// 与密钥异或,输出48bit
		string x = xor_(round_key[i], right_expanded);

		// 8个S盒
		string s = "";
		for (int i = 0; i < 8; i++)
		{
			int index = i * 6; // 每次输入6个bit
			int row = 2 * int(x[index] - '0') + int(x[index + 5] - '0');
			int col = 8 * int(x[index + 1] - '0') + 4 * int(x[index + 2] - '0') + 2 * int(x[index + 3] - '0') + int(x[index + 4] - '0');
			int val = S[i][row][col];
			s += dec_bin(val);
		}
		// 置换运算P
		s = replace(s, P, 32);

		// 将左半与轮函数结果异或
		x = xor_(s, left);

		left = x;

		// 代换
		if (i != 15)
		{
			swap(left, right);
		}
	}

	string combine = left + right;

	// 逆初始置换IP
	string cipher = replace(combine, IP_trans, 64);
	return cipher;
}

vector<string> key_utility(string key)
{
	// 置换选择1去除校验位
	key = replace(key, PC_1, 56);

	// 每轮左移位数
	int shift_table[16] = {1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1};

	string left = key.substr(0, 28);
	string right = key.substr(28, 28);

	vector<string> round_key;

	for (int i = 0; i < 16; i++)
	{

		left = shift_left(left, shift_table[i]);
		right = shift_left(right, shift_table[i]);
		string combine = left + right;
		// 置换选择2
		string RoundKey = replace(combine, PC_2, 48);
		round_key.push_back(RoundKey);
	}
	return round_key;
}

string encode(const unsigned char *txt, const unsigned char *key_char)
{
	string m, key;
	m = hex_bin(txt);
	key = hex_bin(key_char);
	round_key = key_utility(key); // 存储生成的轮函数
	cout << "Encode: " << endl;
	string c = plt_utility(m, round_key);  
	cout << "Cipher Text: " << bin_hex(c) << endl;
	return c;
}

string encode_test(string m, string key)
{
	round_key = key_utility(key); // 存储生成的轮函数
	string c = plt_utility(m, round_key);  
	return c;
}

void decode(const unsigned char *cipher, const unsigned char *key_char)
{
	string c ,key;
	c = hex_bin(cipher);
	key = hex_bin(key_char);
	round_key = key_utility(key);
	cout << "Decode" << endl;
	string text = plt_utility(c, round_key);
	cout << "Plain Text: " << bin_hex(text) << endl;
}
